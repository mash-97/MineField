...
f = Tk::Tile::Entry.new(content){width(7)
								textvariable($feet)
}.grid(:column => 2, :row => 1, :sticky => 'we')

Tk::Tile::Label.new(content){textvariable($meters)}.grid(:column => 2, :row => 2, :sticky => 'we')
Tk::Tile::Button.new(content){text("Calculate")
							command{calculate();puts("In calculation.")}
...
Tk::Tile::Label.new(content){text("YO")}.grid(:column => 1, :row => 4, :sticky => 'w')

Tk::Tile::Label.new(content){text("Shenzow")}.grid(:column => 4, :row => 1, :sticky => 'e')
