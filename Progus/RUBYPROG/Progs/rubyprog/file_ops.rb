#File.exist? will return true if a specific file
#or directory exists (note, you must use two slashes \\)
...
#File.directory? to take differing antions
#if a file is data file or a directory
def dirOrFile2(aName) 
	if File.exist?(aName) then
		puts("#{aName} found")
		if(File.directory?(aName)) then
			puts("#{aName} is a directory")
		else 
