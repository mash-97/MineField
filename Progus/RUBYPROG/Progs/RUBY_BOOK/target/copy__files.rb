...
def copyFiles(source_directory, target_directory)
		...
		Dir.foreach(source_directory){
			...
			if(File.file?(source_file_path) and not File.exist?(target_file_path)) then 
				FileUtils.cp(source_file_path, target_directory)
				
			elsif(File.directory?(source_file_path))

				if(File.exist?(target_file_path) && File.directory?(target_file_path) && !Dir.empty?(source_file_path)) then
					puts("E 1")
