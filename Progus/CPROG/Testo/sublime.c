...
//~ {
	...
	//~ printf("Home: %s\n", mash[getValIndexOf(mash)]);
	//~ printf("Spouse Name: %s\n", spouse_names[getValIndexOf(spouse_names)]);
	//~ printf("Children Numbers: %s\n", children_numbers[getValIndexOf(children_numbers)]);
	//~ printf("Luxury Item: %s\n", luxury_items[getValIndexOf(luxury_items)]);
	
	//~ return 0;
//~ }

//~ void showBits(int length, unsigned int num)
//~ {
	//~ printf("%d ", num);
	//~ for(int i=length-1; i>=0; i--)
		//~ printf("%d", (((num&(1<<i))>>i)&1));
	//~ printf("\n");
//~ }
...
int checkTPrimity(long long unsigned num)
{
	int flag = 0;
	if(!(num&1)) 
	{
